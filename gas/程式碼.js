    ...
    if (data.action === 'update_order_time') {
      ...
      try {
        ...
        if (targetRowIndex !== -1) {
          ...
          let updatedDetails = dataRows[targetRowIndex - 1][11]; // 預設保留原資料
          ...
          const fullOrderData = {
            ...
            specificDetails: updatedDetails, // 🚀 帶入最新的地點詳細資訊，確保 PDF 一併更新
...
/**
 * 【自動化封存系統：年度分頁版】
 * 功能：自動將「工作表 1」中超過一年的訂單，按年份搬移至專屬封存工作表（如：封存_2024）。
 * 建議設定：每月 1 號凌晨執行一次。
 */
function autoArchiveOldOrdersByYear() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sourceSheet = ss.getSheetByName(SHEET_TAB_NAME); // 讀取主工作表
  ...
  oneYearAgo.setFullYear(today.getFullYear() - 1); // 取得「一年前」的今天作為門檻

  const archiveGroups = {}; // 用來存放各年份的資料
  const rowsToKeep = [];    // 用來存放要留在主表的資料 (一年內)

  // 2. 遍歷資料進行分類
  for (let i = 0; i < data.length; i++) {
    const row = data[i];
    const orderDateVal = row[0]; // 第一欄：訂單建立日期
    
    // 將日期值轉換為 JavaScript Date 物件進行比較
  ...
  // 3. 執行搬移動作：按年份寫入對應的工作表
  for (const sheetName in archiveGroups) {
    let targetSheet = ss.getSheetByName(sheetName);
    
    // 如果該年份的工作表還沒建立，則自動建立並設定標題
    if (!targetSheet) {
      ...
      targetSheet.setFrozenRows(1); // 凍結首行
    }
