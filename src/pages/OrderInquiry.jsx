...
const getOrderCreatedTime = (order) => {
  const dateMatch = String(order.orderDate || '').match(/(\d{4})[-/](\d{1,2})[-/](\d{1,2})/);
  const timeSource = `${order.orderTime || ''} ${order.orderDate || ''}`;
  const timeMatch = String(timeSource).replace(/：/g, ':').match(/(\d{1,2}):(\d{2})(?::(\d{2}))?/);
  ...
  const createdAt = getTimestampValue(order.createdAt);
  if (createdAt) return createdAt;

  return getTimestampValue(order.updatedAt);
};

const compareOrdersNewestFirst = (a, b) => {
...
const OrderInquiry = ({ setAlertMsg }) => {
  ...
  const sortedInquiryMatches = useMemo(() => sortOrdersNewestFirst(inqMatches), [inqMatches]);
    ...
    try {
      ...
      if (result.status === 'success') {
        if (result.pdfDownloadUrl && inqData.id) {
          ...
          setInqMatches(prev => sortOrdersNewestFirst(prev.map(order => order.id === inqData.id ? updatedOrder : order)));
        }
      ...
      {inqStatus === 'multiple_matches' ? (
        <div className="bg-pureWhite/95 border border-pureWhite shadow-xl rounded-2xl p-6 md:p-10 mx-auto max-w-2xl relative overflow-hidden animate-[fadeIn_0.5s_ease-out]">
          ...
          <div className="space-y-4">
            {sortedInquiryMatches.map((order) => (
              <div key={order.id || order.orderNumber} onClick={() => {
                setInqData(order);
