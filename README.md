...
- 儲存通知事件、共同管理者，以及每位管理者在各事件中的通知通道設定
- 寫入通知發送紀錄
...
|---|---:|---:|---:|---|---|
| 顧客送出新訂單 | 會 | 會 | 會 | 主旨：`【系統通知】收到新訂單 - 編號 {訂單編號}`。內容：`🍡【新訂單通知】`、訂單編號、訂購人、活動日期時間、報表同步成功 / 失敗、PDF 連結；Email 另附訂單 PDF。 | `create_order` 流程會呼叫 `newOrder` 通知規則；可在後台針對每位管理者調整通道。 |
