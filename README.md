...
| 時機 | 預設 Email | 預設 LINE | 預設 Telegram | 發送通知內容 | 觸發條件 / 備註 |
|---|---:|---:|---:|---|---|
| 顧客送出新訂單 | 會 | 會 | 會 | 主旨：`【系統通知】收到新訂單 - 編號 {訂單編號}`。內容：`🍡【新訂單通知】`、訂單編號、訂購人、活動日期時間、報表同步成功 / 失敗、PDF 連結；Email 另附訂單 PDF。 | `create_order` 流程會呼叫 `newOrder` 通知規則；可在後台改通道與收件人。 |
| 每日出貨提醒 | 不會 | 不會 | 會 | 主旨：`【系統通知】每日出貨提醒 - {日期}`。有訂單時內容包含 `每日出貨提醒 - {日期}`、訂單數、糖葫蘆總數、掃帚租借數、各訂單時間 / 編號 / 訂購人 / 地點 / 品項 / 金額 / PDF；無訂單時內容為 `每日出貨提醒 - {日期}` 與 `明日沒有排定訂單。`。 | 後台啟用每日提醒後，GAS trigger 每天依設定時間執行 `dailyShippingReminder` 通知規則。 |
| 後台修改訂單並重產 PDF | 不會 | 不會 | 不會 | 主旨：`【系統通知】訂單已修改 - 編號 {訂單編號}`。內容：訂單已由管理員修改並重產 PDF、訂單編號、活動日期時間；Email 另附更新後 PDF。 | 對應 `orderUpdated` 通知規則；預設關閉，可在後台啟用。 |
| 後台或顧客補寄 PDF | 不會 | 不會 | 不會 | 主旨：`【系統通知】訂單 PDF 已補寄 - 編號 {訂單編號}`。內容：訂單 PDF 已補寄、訂單編號、收件 Email。 | 對應 `resendPdf` 通知規則；預設關閉，可在後台啟用。 |
| 後台刪除訂單 | 不會 | 不會 | 不會 | 主旨：`【系統通知】訂單已刪除 - 編號 {訂單編號}`。內容：訂單已由管理員刪除、訂單編號。 | 對應 `orderDeleted` 通知規則；預設關閉，可在後台啟用。 |
| Sheets 同步失敗 | 會 | 不會 | 會 | 主旨：`【系統警示】訂單報表同步失敗 - 編號 {訂單編號}`。內容：訂單報表同步失敗、訂單編號、錯誤訊息。 | 對應 `sheetSyncFailed` 通知規則；預設 Email + Telegram。 |
| 儲存每日提醒設定 | 不會 | 不會 | 不會 | 不會立即發送通知。 | 只建立 / 更新 GAS 定時 trigger。 |
